...
# 填写预测变量
# 社会人口学
factor1 = st.radio('户口类型', ['农业户口', '非农户口', '统一居民户口', '不清楚'])
factor2 = st.radio('教育水平', ['小学及以下', '初中毕业', '高中或中专毕业', '大专毕业', '本科及以上', '不清楚'])
factor3 = st.radio('婚姻状态', ['已婚', '未婚', '离异或丧偶', '不清楚'])
factor4 = st.radio('性别', ['男性', '女性'])
factor5 = st.slider('请选择您的年龄', 45, 120)
factor6 = st.slider('您晚上的睡眠时间', -1, 24)
factor7 = st.radio('您每日吃几顿饭？', ['每天大于等于四顿', '每天三顿', '每天两顿', '每天一顿', '不清楚'])
factor8 = st.radio('您一年内喝酒频率', ['每个月超过一次', '每月少于等于一次', '不喝', '不清楚'])
factor9 = st.radio('您是否左胸痛？', ['是', '否', '不清楚'])
factor10 = st.radio('您是否有高血压？', ['是', '否', '不清楚'])
factor11 = st.radio('您是否有血脂异常？', ['是', '否', '不清楚'])
factor12 = st.radio('您是否有糖尿病？', ['是', '否', '不清楚'])
factor13 = st.radio('您是否有慢性肺部疾病？', ['是', '否', '不清楚'])
factor14 = st.radio('您是否有肝脏疾病？', ['是', '否', '不清楚'])
factor15 = st.radio('您是否有心脏病？', ['是', '否', '不清楚'])
factor16 = st.radio('您是否有中风？', ['是', '否', '不清楚'])
factor17 = st.radio('您是否有肾脏疾病？', ['是', '否', '不清楚'])
factor18 = st.radio('您是否有胃部或消化系统疾病？', ['是', '否', '不清楚'])
factor19 = st.radio('您是否有情感及精神方面问题？', ['是', '否', '不清楚'])
factor20 = st.radio('您是否有与记忆相关疾病？', ['是', '否', '不清楚'])
factor21 = st.radio('您是否有关节炎或风湿？', ['是', '否', '不清楚'])
factor22 = st.radio('您是否有哮喘？', ['是', '否', '不清楚'])
factor23 = st.radio('您是否有经历过交通事故或重大意外伤害？', ['是', '否', '不清楚'])
factor24 = st.radio('您是否有确诊青光眼？', ['是', '否', '不清楚'])
factor25 = st.radio('您的听力如何？', ['非常好', '很好', '好', '一般', '不好', '不清楚'])
factor26 = st.radio('您是否牙齿已经掉光？', ['是', '否', '不清楚'])
factor27 = st.radio('您是否经常身体疼痛？', ['是', '否', '不清楚'])
factor28 = st.radio('您是否一年内体重变化超过10斤？', ['是', '否', '不清楚'])
factor29 = st.radio('您小时候的营养情况如何？', ['非常好', '很好', '好', '一般', '不好', '不清楚'])
factor30 = st.radio('您的生活水平如何？', ['非常好', '相对好', '一般', '相对差', '很差', '不清楚'])
factor31 = st.radio('您的工作单位是？', ['政府部门或事业单位', '非盈利结构和企业', '个体户农户居民户', '其他', '不清楚'])
factor32 = st.radio('您家里的洗澡设施是什么样的？', ['统一供热水', '家庭自装热水器', '没有洗澡设施', '不清楚'])
factor33 = st.radio('您家里的供暖设施是什么样的？', ['太阳能', '煤炭或者蜂窝煤', '管道天然气或煤气', '液化石油气', '电', '烧秸秆、柴火', '其他', '不清楚'])
factor34 = st.radio('您家里的室内整洁度如何？', ['非常整洁', '很整洁', '整洁', '一般', '不整洁', '不清楚'])
