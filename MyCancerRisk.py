...
factor19 = st.radio('Do you have emotional or mental issue? (您是否有情感及精神方面问题？)', ['Yes (是)', 'No(否)', 'Not Sure(不清楚)'], index = None)
factor20 = st.radio('Do you have memory-related diseases? (您是否有与记忆相关疾病？)', ['Yes (是)', 'No(否)', 'Not Sure(不清楚)'], index = None)
factor21 = st.radio('Do you have arthritis or rheumatism? (您是否有关节炎或风湿？)', ['Yes (是)', 'No(否)', 'Not Sure(不清楚)'], index = None)
...
# 设置一个按钮用于预测
if st.button('Please click the button to predict.<br>点击进行预测'):
    # 检查是否完成了所有选项
